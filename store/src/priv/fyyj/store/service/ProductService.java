...
import priv.fyyj.store.bean.PageBean;
import priv.fyyj.store.bean.Product;
import priv.fyyj.store.dao.ProductDao;
import priv.fyyj.store.exception.AddProductException;
import priv.fyyj.store.exception.FindProductByIdException;
import priv.fyyj.store.exception.ListProductException;
/**
 * 商品Service
 * @author fyyj
 *
...
public class ProductService {
	...
	public List<Product> findProductByManyCondition(String id, String category, String name,
		...
		try {
			ps = dao.findProductByManyCondition(id, category, name, minprice, maxprice);
			return ps;
		} catch (SQLException e) {
			e.printStackTrace();
		}
