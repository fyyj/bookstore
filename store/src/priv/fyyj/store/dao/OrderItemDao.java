...
public class OrderItemDao {
	...
	public void addOrderItem(Order order) throws SQLException {
		...
		runner.batch(DataSourceUtil.getConnection(), sql, params);
	}

	// 根据订单查找订单项.并将订单项中商品查找到。
	public List<OrderItem> findOrderItemByOrder(final Order order)
			throws SQLException {
		String sql = "select * from orderItem,Products where products.id="
				+ "orderItem.product_id and order_id=?";
	...
	public void delOrderItems(String id) throws SQLException {
		...
		QueryRunner runner=new QueryRunner();
		runner.update(DataSourceUtil.getConnection(),sql,id);
	}
}
